...
export default {
  ...
  methods:{
    ...
    trademarkHandler(trademark){
      ...
      // 一个问题：在父组件发请求，还是子组件发请求？   父组件
      // 原因：因为父组件中searchParams参数是带给服务器的参数，子组件把点击的品牌信息需要给父组件传递过去
      // 解决：自定义事件
      this.$emit('trademarkInfo',trademark);     //触发自定义事件
    },

    // 平台售卖属性值的点击事件
    attrValueHandler(attr,attrValue){
      // 向服务器发送请求。--在父组件使用自定义事件
      this.$emit('attrValueInfo',attr,attrValue);     //触发自定义事件
    }
  }
}
</script>
